...
class PxMATRIX : public Adafruit_GFX {
 public:
  ...
  inline void setMuxDelay(uint8_t mux_delay_A, uint8_t mux_delay_B, uint8_t mux_delay_C, uint8_t mux_delay_D,
                          uint8_t mux_delay_E);
  ...
  // Set the color depth
  inline void setColorDepth(uint8_t color_depth);

  // Set SPI frequency
  inline void setSpiFrequency(uint32_t spi_frequency);

  // Get the total amount of pixels controlled by this driver
  inline uint32_t getPixelCount();

 private:
  // the display buffer for the LED matrix
  uint8_t PxMATRIX_buffer[PxMATRIX_MAX_COLOR_DEPTH][buffer_size];
#ifdef PxMATRIX_double_buffer
  uint8_t PxMATRIX_buffer2[PxMATRIX_MAX_COLOR_DEPTH][buffer_size];
#endif
...
inline void PxMATRIX::setColorOrder(color_orders color_order) { _color_order = color_order; }

inline void PxMATRIX::setMuxDelay(uint8_t mux_delay_A, uint8_t mux_delay_B, uint8_t mux_delay_C, uint8_t mux_delay_D,
                                  uint8_t mux_delay_E) {
...
inline void PxMATRIX::setScanPattern(scan_patterns scan_pattern) { _scan_pattern = scan_pattern; }
...
inline void PxMATRIX::setRotate(bool rotate) { _rotate = rotate; }

inline void PxMATRIX::setFlip(bool flip) { _flip = flip; }

inline void PxMATRIX::setFastUpdate(bool fast_update) { _fast_update = fast_update; }

inline void PxMATRIX::setBrightness(uint8_t brightness) { _brightness = brightness; }
...
inline void PxMATRIX::drawPixel(int16_t x, int16_t y, uint16_t color) { drawPixelRGB565(x, y, color); }

inline void PxMATRIX::showBuffer() { _active_buffer = !_active_buffer; }
...
inline void PxMATRIX::fillMatrixBuffer(int16_t x, int16_t y, uint8_t r, uint8_t g, uint8_t b, bool selected_buffer) {
  ...
  uint8_t(*PxMATRIX_bufferp)[PxMATRIX_MAX_COLOR_DEPTH][buffer_size] = &PxMATRIX_buffer;
...
inline void PxMATRIX::begin() { begin(8); }
...
void PxMATRIX::display(uint16_t show_time) {
  ...
  uint8_t(*bufferp)[PxMATRIX_MAX_COLOR_DEPTH][buffer_size] = &PxMATRIX_buffer;
    ...
    if (_driver_chip == SHIFT) {
      if ((_fast_update) && (_brightness == 255)) {
        ...
        if (i < _row_pattern - 1) {
          ...
          SPI_TRANSFER(&(*bufferp)[((_display_color + 1) % _color_depth)][0], _send_buffer_size);
        }
...
#ifdef PxMATRIX_double_buffer
  if (selected_buffer)
    memset(PxMATRIX_buffer2, 0, PxMATRIX_MAX_COLOR_DEPTH * buffer_size);
  else
    memset(PxMATRIX_buffer, 0, PxMATRIX_MAX_COLOR_DEPTH * buffer_size);
#else
  memset(PxMATRIX_buffer, 0, PxMATRIX_MAX_COLOR_DEPTH * buffer_size);
#endif
}
